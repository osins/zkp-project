# Project Structure

English | [简体中文](STRUCTURE_CN.md)

```
zkp-project/
│
├── 📄 README.md                      # Main documentation
├── 📄 README_CN.md                   # Main documentation (Chinese)
├── 📄 LICENSE                        # MIT License
├── 📄 package.json                   # Root package.json (workspace)
├── 📄 .gitignore                     # Git ignore configuration
├── 📄 STRUCTURE.md                   # This file (Project structure)
├── 📄 STRUCTURE_CN.md                # Project structure (Chinese)
├── 📄 COMMANDS.md                    # Command reference
├── 📄 COMMANDS_CN.md                # Command reference (Chinese)
├── 📄 QUICKSTART.md                  # Quick start guide
├── 📄 QUICKSTART_CN.md               # Quick start guide (Chinese)
├── 📄 PROJECT_SUMMARY.md             # Project summary
├── 📄 PROJECT_SUMMARY_CN.md          # Project summary (Chinese)
│
├── 📁 rust-prover/                   # Rust proof generator (Halo2)
│   ├── 📄 Cargo.toml                 # Rust dependencies configuration
│   ├── 📄 build_wasm.sh              # WASM build script
│   ├── 📄 README.md                  # Module documentation
│   ├── 📄 README_CN.md               # Module documentation (Chinese)
│   ├── 📄 CHANGELOG.md               # Changelog
│   ├── 📄 .gitignore
│   │
│   ├── 📁 src/
│   │   ├── 📄 lib.rs                 # WASM interface and core logic
│   │   └── 📄 circuit.rs             # Halo2 circuit definition (x² = y)
│   │
│   ├── 📁 test/
│   │   ├── 📄 test-wasm.js           # Complete test suite (15 tests)
│   │   ├── 📄 test-simple.js         # Simple debugging tests
│   │   ├── 📄 README.md              # Test documentation
│   │   ├── 📄 WASM_TEST_SUCCESS.md   # Test report
│   │   └── 📄 test-results.txt       # Test results
│   │
│   └── 📁 pkg/                       # WASM output (generated by wasm-pack)
│       ├── 📄 rust_prover.js
│       ├── 📄 rust_prover_bg.wasm
│       └── 📄 rust_prover.d.ts
│
├── 📁 circom-circuits/               # Circom circuits
│   ├── 📄 package.json               # Node dependencies
│   ├── 📄 .gitignore
│   │
│   ├── 📁 circuits/
│   │   └── 📄 example.circom         # Circom circuit (multiplication verification)
│   │
│   ├── 📁 scripts/
│   │   ├── 📄 build_circuit.sh       # Circuit compilation script
│   │   └── 📄 test_circuit.js        # Circuit testing script
│   │
│   └── 📁 build/                     # Build output (gitignored)
│       ├── 📄 example.r1cs           # R1CS constraint file
│       ├── 📄 example_final.zkey     # Proving key
│       ├── 📄 verification_key.json  # Verification key
│       ├── 📄 Verifier.sol           # Generated Solidity verifier
│       ├── 📄 proof.json             # Sample proof
│       └── 📄 calldata.txt           # Solidity calldata
│
├── 📁 node-sdk/                      # Node.js/TypeScript SDK
│   ├── 📄 package.json
│   ├── 📄 tsconfig.json
│   ├── 📄 .env.example
│   ├── 📄 .gitignore
│   │
│   ├── 📁 src/
│   │   ├── 📄 index.ts               # SDK main entry
│   │   ├── 📄 proverClient.ts        # Proof generation client
│   │   ├── 📄 verifierClient.ts      # Verification client
│   │   └── 📄 contractClient.ts      # Smart contract interaction client
│   │
│   ├── 📁 scripts/
│   │   ├── 📄 generateProof.ts       # Proof generation script
│   │   └── 📄 verifyProof.ts         # Proof verification script
│   │
│   └── 📁 dist/                      # TypeScript compilation output
│
├── 📁 smart-contracts/               # Solidity smart contracts
│   ├── 📄 package.json
│   ├── 📄 hardhat.config.js          # Hardhat configuration
│   ├── 📄 .env.example
│   ├── 📄 .gitignore
│   │
│   ├── 📁 contracts/
│   │   ├── 📄 Verifier.sol           # Groth16 Verifier contract
│   │   └── 📄 ZKPApplication.sol     # ZKP application example contract
│   │
│   ├── 📁 scripts/
│   │   ├── 📄 deploy.js              # Contract deployment script
│   │   └── 📄 verify-on-chain.js     # On-chain verification script
│   │
│   ├── 📁 test/
│   │   └── 📄 Verifier.test.js       # Contract testing
│   │
│   ├── 📁 artifacts/                 # Hardhat compilation output
│   └── 📁 cache/                     # Hardhat cache
│
├── 📁 backend/                       # Backend API service (optional)
│   ├── 📄 package.json
│   ├── 📄 tsconfig.json
│   ├── 📄 .env.example
│   ├── 📄 .gitignore
│   │
│   ├── 📁 src/
│   │   └── 📄 server.ts              # Express.js API server
│   │
│   └── 📁 dist/                      # TypeScript compilation output
│
├── 📁 scripts/                       # Global scripts
│   ├── 📄 setup.sh                   # Project initialization script
│   ├── 📄 build-all.sh               # Build all modules
│   ├── 📄 test-all.sh                # Run all tests
│   ├── 📄 demo.sh                    # Demo script
│   └── 📄 clean.sh                   # Clean build files
│
└── 📁 reports/                       # Validation reports
    ├── 📄 20251108150155_validation_report.yaml
    └── 📄 20251108154606_validation_report.yaml
```

## 📊 File Statistics

| Module | File Count | Primary Language |
|--------|------------|------------------|
| rust-prover | 14 | Rust |
| circom-circuits | 6 | Circom, JavaScript |
| node-sdk | 10 | TypeScript |
| smart-contracts | 11 | Solidity, JavaScript |
| backend | 5 | TypeScript |
| scripts | 6 | Bash |
| Documentation | 20+ | Markdown |
| **Total** | **72+** | Multi-language |

## 🔑 Key File Descriptions

### Rust Prover
- `lib.rs`: WASM interface exports and core proof/verification logic
- `circuit.rs`: Halo2 circuit definition, implements x² = y verification
- `test/test-wasm.js`: Complete test suite (15 test cases)
- `test/test-simple.js`: Simple debugging tests

### Circom Circuits
- `example.circom`: Circom circuit, multiplication verification (a × b = c)
- `build_circuit.sh`: Complete compilation process (R1CS → zkey → Verifier.sol)
- `test_circuit.js`: Circuit testing and proof generation

### Node SDK
- `proverClient.ts`: Encapsulates snarkjs proof generation
- `verifierClient.ts`: Off-chain verification interface
- `contractClient.ts`: ethers.js on-chain interaction

### Smart Contracts
- `Verifier.sol`: Groth16 verifier (generated by snarkjs)
- `ZKPApplication.sol`: Application example (points reward system)
- `deploy.js`: Hardhat deployment script

### Backend
- `server.ts`: RESTful API service
  - `POST /api/proof/generate`
  - `POST /api/proof/verify`
  - `POST /api/proof/export-calldata`

## 🔄 Workflow

```
1. Design Circuit
   ↓
2. Compile Circuit (build_circuit.sh)
   → Generate R1CS, WASM, zkey, Verifier.sol
   ↓
3. Generate Proof (generateProof.ts)
   → Use snarkjs + circuit
   ↓
4. Off-chain Verification (verifyProof.ts)
   → Fast verification (no blockchain needed)
   ↓
5. Deploy Contract (deploy.js)
   → Hardhat deployment to chain
   ↓
6. On-chain Verification (verify-on-chain.js)
   → Verify through smart contract
```

## 📦 Dependencies

```
node-sdk ─┬─→ circom-circuits (build outputs)
          └─→ smart-contracts (ABI)

smart-contracts ──→ circom-circuits (Verifier.sol)

backend ──→ circom-circuits (build outputs)

rust-prover (independent module)
```